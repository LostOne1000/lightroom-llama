...
function client.fetchModels(prefs)
    ...
    logger:warn("Using default model: " .. client.defaultModel)
...
function client.generate(photo, userInstruction, currentData, useCurrentData,
    ...
    -- 4. Send HTTP POST
    local url = client.getBaseUrl(LrPrefs.prefsForPlugin()) .. "/api/generate"
    local jsonPayload = JSON:encode(requestBody)

    local response = LrHttp.post(url, jsonPayload, {{
        field = "Content-Type",
